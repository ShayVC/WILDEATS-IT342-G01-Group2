...
              <ButtonGroup isAdmin={isAdmin}>
                <ViewButton to={`/shops/${shop.shop_id}`} isAdmin={isAdmin}>View</ViewButton>
                
...
// 4. Update ButtonGroup to use flex properties conditionally
const ButtonGroup = styled.div`
  display: flex;
  gap: 0.5rem;
  
  /* If the user is not an admin, the View button takes up the full width (flex: 1) */
  ${props => !props.isAdmin && `
    ${ViewButton} {
      flex: 3; /* Make View button take up all space */
    }
  `}
`;

// 5. Update ViewButton to handle full width when admin buttons are hidden
...
  }
  
  /* Ensure ViewButton takes full space when admin buttons are not rendered */
  ${({ isAdmin }) => !isAdmin && `
    flex: 1;
  `}
`;
