...
export const createFoodItem = async (foodItemData, shopId) => {
    try {
        const response = await axios.post(API_URL, {
            itemName: foodItemData.itemName || foodItemData.name,
            itemDescr: foodItemData.itemDescr || foodItemData.description || '',
            itemImageURL: foodItemData.itemImageURL || foodItemData.imageURL || '',
            price: foodItemData.price,
            isAvailable: foodItemData.isAvailable !== undefined ? foodItemData.isAvailable : true
        }, {
            params: { shopId },
            headers: getAuthHeader()
        });
        ...
        const newFoodItem = {
            ...
            id: newId,
            itemName: foodItemData.itemName || foodItemData.name,
            name: foodItemData.itemName || foodItemData.name,
            itemDescr: foodItemData.itemDescr || foodItemData.description || '',
            description: foodItemData.itemDescr || foodItemData.description || '',
            price: parseFloat(foodItemData.price) || 0,
            isAvailable: foodItemData.isAvailable !== undefined ? foodItemData.isAvailable : true,
            available: foodItemData.isAvailable !== undefined ? foodItemData.isAvailable : true,
...
export const updateFoodItem = async (id, foodItemData) => {
    try {
        ...
        const index = MOCK_FOOD_ITEMS.findIndex(item => item.id === parseInt(id) || item.itemId === parseInt(id));
        ...
        MOCK_FOOD_ITEMS[index] = {
            ...MOCK_FOOD_ITEMS[index],
            itemName: foodItemData.itemName || foodItemData.name || MOCK_FOOD_ITEMS[index].itemName || MOCK_FOOD_ITEMS[index].name,
            name: foodItemData.itemName || foodItemData.name || MOCK_FOOD_ITEMS[index].itemName || MOCK_FOOD_ITEMS[index].name,
            itemDescr: foodItemData.itemDescr || foodItemData.description || MOCK_FOOD_ITEMS[index].itemDescr || MOCK_FOOD_ITEMS[index].description,
            description: foodItemData.itemDescr || foodItemData.description || MOCK_FOOD_ITEMS[index].itemDescr || MOCK_FOOD_ITEMS[index].description,
            price: parseFloat(foodItemData.price) || MOCK_FOOD_ITEMS[index].price,
            isAvailable: foodItemData.isAvailable !== undefined ? foodItemData.isAvailable : (MOCK_FOOD_ITEMS[index].isAvailable !== undefined ? MOCK_FOOD_ITEMS[index].isAvailable : MOCK_FOOD_ITEMS[index].available),
            available: foodItemData.isAvailable !== undefined ? foodItemData.isAvailable : (MOCK_FOOD_ITEMS[index].isAvailable !== undefined ? MOCK_FOOD_ITEMS[index].isAvailable : MOCK_FOOD_ITEMS[index].available)
        };
...
export const deleteFoodItem = async (id) => {
    try {
        ...
        MOCK_FOOD_ITEMS = MOCK_FOOD_ITEMS.filter(item => item.id !== parseInt(id) && item.itemId !== parseInt(id));
...
export const updateFoodItemAvailability = async (id, isAvailable) => {
    try {
        ...
        // Update availability
        MOCK_FOOD_ITEMS[index].isAvailable = isAvailable;
        MOCK_FOOD_ITEMS[index].available = isAvailable;
