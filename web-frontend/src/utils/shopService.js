...
export const getMyShops = async () => {
    try {
        const response = await axios.get(`${API_URL}/my-shops`, {
...
export const approveShop = async (shopId) => {
    try {
        const response = await axios.put(`${API_URL}/${shopId}/approve`, {}, {
            headers: getAuthHeader()
        });
        return response.data;
    } catch (error) {
...
// Reject a shop (ADMIN only) - uses delete endpoint
export const rejectShop = async (shopId) => {
    try {
        ...
        console.error(`Error rejecting shop ${shopId}:`, error);
        throw error;
    }
};
